-- Load data from HDFS
inputDialogues4 = LOAD 'hdfs:///user/divyal/inputs/episodeIV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues5 = LOAD 'hdfs:///user/divyal/inputs/episodeV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues6 = LOAD 'hdfs:///user/divyal/inputs/episodeVI_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
...
rmf hdfs:///user/divyal/outputs;

--Store result in HDFS
STORE namesOrdered INTO 'hdfs:///user/divyal/outputs' USING PigStorage('\t');
